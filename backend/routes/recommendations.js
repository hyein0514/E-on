...
try {
    ...
    SELECT 
        c.*,
        -- 관심과 진로 매칭 여부를 점수로 계산
        CASE
        WHEN ci.interest_id IS NOT NULL AND cv.vision_id IS NOT NULL THEN 2
        WHEN ci.interest_id IS NOT NULL OR cv.vision_id IS NOT NULL THEN 1
        ELSE 0
        END AS match_score
    FROM Challenge c
